...
var Wrts = (function () {
	...
	Wrts.prototype.getUserInfo = function (callback) {
		...
		request.get('https://www.wrts.nl/user.json', wrapRequestCallback(function (err, res) {
			if (err) {
				callback(err, null);
			} else {
	...
	Wrts.prototype.getLists = function (callback) {
		request.get('https://www.wrts.nl/lists/', wrapRequestCallback(function (err, res) {
			if (err) {
				callback(err, null);
			} else {
	...
	Wrts.prototype.getResults = function (callback) {
		request.get('https://www.wrts.nl/results/', wrapRequestCallback(function (err, res) {
			if (err) {
				callback(err, null);
			} else {
...
module.exports = function (email, password, callback) {
	...
	request.get('https://www.wrts.nl/user.json', function (err, response) {
		if (err) {
			callback(err, null);
